# 🤖 ПОЛНЫЙ ПРОМПТ ДЛЯ AI АГЕНТА: Реализация Payment Consent Flow

> **Использование**: Скопируй этот промпт и передай агенту. Все необходимые инструкции и контекст включены.
...
**Референс**: Изучи `initiate_product_consent()` строки 274-360 в том же файле
...
**Референс**: Изучи `initiate_product_consent()` строки 77-132 в том же файле
...
### Обязательные файлы (изучи перед началом):
...
### Паттерн создания consent в `obr_client.py`

Смотри на `initiate_product_consent()` (строки 274-360):
1. Получить токен: `bank_token = await self._get_bank_token()`
2. Подготовить headers: `headers = await self._get_common_headers(bank_token)`
3. Подготовить несколько вариантов payload
4. Попробовать каждый вариант в цикле
5. Обработать ответ, извлечь `consent_id`, `request_id`, `status`
6. Вернуть `ConsentInitResult` или `None`

### Паттерн обертки в `consents.py`

Смотри на `initiate_product_consent()` (строки 77-132):
1. Получить bank config
2. Создать client через `bank_client()`
3. Вызвать низкоуровневый метод `client.initiate_payment_consent()`
4. Сохранить в БД через `save_consent()` с `consent_type="payments"`
5. Обработать ошибки gracefully
...
1. **Endpoint может отличаться**: Банки могут использовать `/payment-consents/request` или `/payment-consent-requests`. Попробуй оба варианта.
...
1. Изучи `initiate_product_consent()` в `obr_client.py` (строки 274-360)
2. Адаптируй его для payment consent
3. Добавь метод `initiate_payment_consent()` после строки 360
