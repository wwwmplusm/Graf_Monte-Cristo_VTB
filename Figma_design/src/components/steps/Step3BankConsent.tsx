...
interface Step3BankConsentProps {
  onNext: (consents: Record<string, string>, hasDebts: boolean) => void;
  onBack: () => void;
  ...
  useEffect(() => {
    if (state === "success") {
      const timer = setTimeout(() => {
        if (isLastBank) {
          // All banks connected, simulate checking for debts
          // In real app: this would be fetched from API after getting all consents
          const hasDebts = Math.random() > 0.3; // 70% chance of having debts
          onNext(consents, hasDebts);
        } else {
          // Move to next bank
